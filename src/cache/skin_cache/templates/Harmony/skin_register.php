...
class skin_register extends CTM_Command
{
	//============================================
	...
	{
		global $confirm_error, $link_id;  // PHP Globals

		$CTM_HTML = NULL;
		$CTM_HTML .= "	<div id=\"Verify\" style=\"display: none;\"></div>
    <div class=\"box-content\">
    	<div class=\"header\"><span>{$this->lang->words['Register']['Confirm']['Title']}</span></div>
        ".(empty($link_id) ? "<form name=\"ConfirmRegister\" id=\"ConfirmRegister\" class=\"frm\">
        ...
        </form>" : "".(!empty($confirm_error) ? "<div class=\"error-box\">{$confirm_error}</div>" : "<p>{$this->lang->words['Register']['Confirm']['Messages']['Link']['Success']}</p>")."")."
    </div>
    <div id=\"Command\"></div>";
		return $CTM_HTML;
	}
	...
	{
		global $register_module;  // PHP Globals
    ...
    $(function()
    {
        ...
        ".($this->settings['REGISTER']['REGISTER_PID'] == true ? "$(\"#PersonalID\").blur(function()
        {
            if($(this).val().length < 1)
        ...
        $(\"#CompleteRegister\").click(function()
        {
            ...
            else
            {
				...
				".($this->settings['REGISTER']['REGISTER_PID'] == true ? "if($(\"#PersonalID\").val().length < 1) message += \"&raquo; {$this->lang->words['Register']['Register']['Messages']['NULL_PID']}<br />\";" : NULL)."
                ...
                else
                {
					...
					".($this->settings['REGISTER']['REGISTER_PID'] == true ? "if($(\"#PersonalID\").val().length != 7)
						message += \"&raquo; {$this->lang->words['Register']['Register']['Messages']['Error_PIDLength']}<br />\";" : NULL)."
					...
					".($this->settings['REGISTER']['REGISTER_PID'] == true ? "if(isNaN($(\"#PersonalID\").val()))
						message += \"&raquo; {$this->lang->words['Register']['Register']['Messages']['Error_PIDWords']}<br />\";" : NULL)."
		...
		<form name=\"RegisterAccount\" id=\"RegisterAccount\" class=\"frm\">
        	...
        	<table width=\"100%\" border=\"0\" align=\"center\" cellpadding=\"7\">
                <tbody>
                    ...
                    ".($this->settings['REGISTER']['REGISTER_PID'] == true ? "<tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <select name=\"SecureQuestion\" id=\"SecureQuestion\">
                                ...
                                ".$this->loop__registerAccount__0x00()."
                            </select>
                        </td>
                    </tr>
                    <tr>
        	...
        	".($register_module['countBonus'] > 0 ? "<h4>{$this->lang->words['Register']['Register']['RegisterBonus']['Title']}</h4>
            <table width=\"100%\" border=\"0\" align=\"center\" cellpadding=\"7\">
                <tbody>
                    ".($this->settings['REGISTER']['VAULT_BONUS']['SWITCH'] == true ? "<tr>
                        ...
                        <td width=\"60%\">
                            <select name=\"VaultBonus\" id=\"VaultBonus\">
                                ...
                                ".$this->loop__registerAccount__0x01()."
                            </select>
                        </td>
                    </tr>" : NULL)."
                    ".($this->settings['REGISTER']['VIP']['SWITCH'] == true ? "<tr>
                    ...
                    ".($this->settings['REGISTER']['COIN']['SWITCH'] == true ? "<tr>
	...
	//============================================
	/* registerAccount : 0x00 */
	private function loop__registerAccount__0x00()
	{
		$content = NULL;

		if(count($this->lang->words['Register']['Register']['AccountDetails']['SecureQuestion']['Questions']) > 0)
		{
			foreach($this->lang->words['Register']['Register']['AccountDetails']['SecureQuestion']['Questions'] as $question)
			{
				$content .= "<option value=\"{$question}\">{$question}</option>
                                ";
			}
		}
		return rtrim($content);
	}
	/* registerAccount : 0x01 */
	private function loop__registerAccount__0x01()
	{
		$content = NULL;

		if(count($this->settings['REGISTER']['VAULT_BONUS']['OPTIONS']) > 0)
		{
			foreach($this->settings['REGISTER']['VAULT_BONUS']['OPTIONS'] as $key => $value)
			{
...
$callSkinCache = new skin_register();
