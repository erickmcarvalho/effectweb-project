...
class CTMCommand_Functions extends CTM_Command
{
	...
	*/
	public function GetCharImage($data)
	{
		...
		$url = CTM_ROOT_AREA == "admin" ? str_replace(ADMINCP_DIRECTORY."/", NULL, $url) : $url;

		$dir = $this->settings['WEBDATA']['UPLOADS']['DIRECTORY']['CHARIMAGE'];
		
		if(strlen($data) < 2 || !file_exists($dir.$data))
		{
			$template = CTM_ROOT_AREA == "admin" ? EffectWebInfo::DEFAULT_TEMPLATE : $this->output->template;
